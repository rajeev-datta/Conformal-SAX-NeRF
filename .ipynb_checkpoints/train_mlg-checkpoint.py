...
from src.loss import calc_mse_loss, BatchedPinballLoss
...
class BasicTrainer(Trainer):
    def __init__(self):
        ...
        wandb.init(
            project=os.environ.get("WANDB_PROJECT", f"{cfg['exp']['expname']} (regular)"),
            name=os.environ.get("WANDB_NAME", f"lr-{cfg['train']['lrate']}-bs-{cfg['train']['n_batch']}"),
            config={
                "batch_size": cfg['train']['n_batch'],
                "main_lr": cfg['train']['lrate'],
                "epochs": cfg['train']['epoch'],
            },
        )

        self.pinball = BatchedPinballLoss(reduction="mean")

        self.eval_quantile = cfg["train"].get("eval_quantile", 0.5)
        self.eval_quantiles = cfg["train"].get("eval_quantiles", [0.05, 0.5, 0.95])
    ...
    def compute_loss(self, data, global_step, idx_epoch):
        ...
        # print("INPUT")
        # print("GT Projection: ", projs.shape)
        # print("Rays: ", rays.shape)

        curr_quantiles = None

        if self.uncertainty:
            batch_size = rays.shape[0]  # one quantile per ray
            curr_quantiles = torch.rand(batch_size, device=device, dtype=torch.float32)
            curr_quantiles = curr_quantiles.clamp_(1e-7, 1.0 - 1e-7)
            ret = render(rays, self.net, self.net_fine, self.conf["render"]["n_samples"], self.conf["render"]["n_fine"], self.conf["render"]["perturb"],
                        self.conf["render"]["netchunk"], self.conf["render"]["raw_noise_std"], curr_quantiles)
        else:
            ret = render(rays, self.net, self.net_fine, self.conf["render"]["n_samples"], self.conf["render"]["n_fine"], self.conf["render"]["perturb"],
                        self.conf["render"]["netchunk"], self.conf["render"]["raw_noise_std"])
        
        # stx()
        # stx()
        projs_pred = ret["acc"]

        # print("OUT: ", projs_pred.shape)

        loss = {"loss": 0.}

        if self.uncertainty:
            loss = self.pinball(loss, projs_pred, projs, curr_quantiles)
        else:
            loss = calc_mse_loss(loss, projs, projs_pred)

        # Log
        for ls in loss.keys():
            self.writer.add_scalar(f"train/{ls}", loss[ls].item(), global_step)
            wandb.log({f"train/{ls}" : loss[ls].item()})
    ...
    def eval_step(self, global_step, idx_epoch):
        ...
        for i in tqdm(range(0, rays.shape[0], self.n_rays)):
            rays_chunk = rays[i:i + self.n_rays]
            ...
            projs_pred.append(out)
    
        projs_pred = torch.cat(projs_pred, 0).reshape(N, H, W)
    
        image = self.eval_dset.image
    
        # For full 3D voxel evaluation, also use median quantile if UQ is enabled.
        if self.uncertainty:
            voxels = self.eval_dset.voxels.to(device)
            vox_q = torch.full(
                voxels.shape[:-1],
                0.5,
                device=device,
            ...
            image_pred = run_network(
                self.eval_dset.voxels.to(device),
                self.net_fine if self.net_fine is not None else self.net,
                self.netchunk,
            )
