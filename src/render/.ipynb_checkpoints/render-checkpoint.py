...
def render(rays, net, net_fine, n_samples, n_fine, perturb, netchunk, raw_noise_std, quantiles=None):
    n_rays = rays.shape[0]

    if quantiles is not None:
        quantiles = quantiles.view(-1)
        if quantiles.shape[0] != n_rays:
            raise ValueError(
                f"quantiles must have shape [{n_rays}], got {tuple(quantiles.shape)}"
            )
    ...
    # Expand ray-level quantiles to point-level quantiles for coarse pass.
    if quantiles is not None:
        quantiles_coarse = quantiles[:, None].expand(n_rays, n_samples)
    else:
        quantiles_coarse = None

    raw = run_network(pts, net, netchunk, quantiles=quantiles_coarse)
    ...
    if net_fine is not None and n_fine > 0:
        ...
        z_samples = sample_pdf(z_vals_mid, weights[..., 1:-1], n_fine, det=(perturb == 0.0))
        z_samples = z_samples.detach()
        ...
        if quantiles is not None:
            quantiles_fine = quantiles[:, None].expand(n_rays, z_vals.shape[1])
        else:
            quantiles_fine = None

        raw = run_network(pts, net_fine, netchunk, quantiles=quantiles_fine)
        acc, _ = raw2outputs(raw, z_vals, rays_d, raw_noise_std)
...
def run_network(inputs, fn, netchunk, quantiles=None):
    """
    Prepares inputs and applies network fn.

    inputs:
        training: [N_rays, N_samples, 3]
        eval 3D:  [..., 3]

    quantiles:
        either None
        or shape matching inputs without the last coord dim:
            [N_rays, N_samples] for ray samples
            or [...] for voxel queries
    """
    uvt_flat = torch.reshape(inputs, [-1, inputs.shape[-1]])  # [..., 3] -> [N_total, 3]

    if quantiles is not None:
        quantiles_flat = torch.reshape(quantiles, [-1])
        if quantiles_flat.shape[0] != uvt_flat.shape[0]:
            raise ValueError(
                f"quantiles_flat has shape {quantiles_flat.shape}, "
                f"but flattened inputs have shape {uvt_flat.shape}"
            )
    else:
        quantiles_flat = None
    ...
    for i in range(0, uvt_flat.shape[0], netchunk):
        x_chunk = uvt_flat[i:i + netchunk]

        if quantiles_flat is None:
            out_chunk = fn(x_chunk)
        else:
            q_chunk = quantiles_flat[i:i + netchunk]
            out_chunk = fn(x_chunk, q_chunk)

        outputs.append(out_chunk)

    out_flat = torch.cat(outputs, 0)
